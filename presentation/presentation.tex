...
\begin{frame}
  \frametitle{Fuzzing Large Project}
  How does these two tools perform on the large scale project?
  
  \pause
  We tried them on apache,
...
Both Quickchick and FuzzChick successfully run on the apache server.

\pause{}
Bad news:

We failed to go deeper than the client side on the server.
...
Background: \\~\\
We are using the patch which is mentioned in AFL, so that the apache server will take in a string as its argument. \\~\\
\pause{}
But this patch used pthread to launch the server and what is happening at the backend is obscure (aka. neither humanbeing nor AFL knows what it is doing). \\~\\
\pause{}
The pthread is not AFL instrumented and AFL cannot analysis the underlying path in the blackbox and do its state driven fuzzing technique. (GDB results said so :p)
...
\begin{itemize}
    \item pros: Quickchick runs pretty fast at generating test cases.
    
    \item Cons: Quickchick fails to capture the successful case I want when we generate 10000 random strings. (That sounds natural I guess).
\end{itemize}
...
\begin{itemize}
    \item pros: FuzzChick runs AFL and AFL does not generate random string, but it can cheat on having some testscript that people wrote.
    
    \item cons: It runs pretty slowly (~1.2s per testcase). Maybe the string it comes up with is meaningful to the server.
\end{itemize}
